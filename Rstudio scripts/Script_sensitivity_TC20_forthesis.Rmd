...
This script is to run analysis from the normative model when we include 20% of the TC in the test set instead of 10% and see if this changes. So this script is for the sensitivity analysis. We will load the clustering data from the other script. Make sure to carefully run everything.
...
load("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/workspace.RData")
```
...
Deviation_scores <- read_csv("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/results/blr/powell/outputs/Deviation_scores.csv", col_types = cols(...1 = col_skip()))
```
...
Subgroups
```{r comparison abs deviation score subgroups, cache=TRUE}
...
Z_df <- read.csv("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/results/Z_df.csv")
...
Compare atypicality index between autism and TC
...
these groups. We want different distributions for the groups.
```{r prepare dataframes for atypicality index subgroups}
#Split dataframes into subgroups
...
Compare atypicality index between subgroups
```{r compare atypicality index subgroups}
# Positive
## assumptions normality:
...
Visualize the atypicality index
```{r plot atypicality index subgroups}
...
ggplot(counts_all, aes_string(x = "DX_GROUP", y = "counts")) +
  ...
  scale_fill_manual(values = c("#5b78e2", "#b30d2e"), name = "Group")
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_sensitivity_analysis/", ROI,"dif_count_diagnosis.png"))
...
ggplot(counts_ASD, aes_string(x = "classification_model", y = "counts")) +
  ...
  scale_fill_manual(values = c("#5b78e2", "#b30d2e"), name = "Subgroups")
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_sensitivity_analysis/", ROI,"dif_count_subgroups.png"))
...
write.table(Z_df, "/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/results/P_df.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)

```

We want to get brain maps for the different subgroups as well. So let's
save the data for each subgroup separately so it's easy to handle in
python.
```{r data for statistic brain maps subgroups}
...
write.table(Z_df_1, "/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/results/Z_df_subtype1.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)

write.table(Z_df_2, "/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/results/Z_df_subtype2.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)
```
...
Load sub IDs that are in the TCtest set (with 10% of TC used for test)
```{r}
subids_test_TC20 <- read.csv("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model_sensitivity_analysisTC20/subids_test_TC20.csv")
