...
-   Make sure you run the libraries. Some functions will not give an error but do not work properly.
-   We switch between using R and python. When loading something specific from the model is not working, it probably has not been run on python yet. 
...
Plots that show the distribution of the variables. This plot is not shown in the thesis. 
...
ggsave("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Cluster plots/3plots_Subtypes_Subscores.png")
```
...
the clusters change a lot, this could indicate instable clusters. A cluster is defined as stable when the same number of clusters are found.
...
From this part on we use the data from the normative model. This model is run in Python. So make sure you run the model before you start this part of the code. 
...
We will make a dataframe that we will use in the classification in python.
```{r dataframe SVM}
...
We compare the deviation scores from the normative model between autism and TC. We will only show and save the plot if the area is signficiant after FDR correction.
...
for (i in 1:150){
  ...
  if (p_values_diag[i, 3] < 0.05) {
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/", p_values_diag$ROI[i],"Sig_devscores_diagnosis.png"))
  }
}
```
...
```{r plot Atypicality score diagnosis}
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "positive_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_positive_diag.png"))
...
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "negative_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_negative_diag.png"))
...
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "absolute_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_absolute_diag.png"))
...
```{r plot Atypicality index subtypes}
ggplot(Z_max_plot, aes_string(x = "group", y = "positive_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_positive_subtypes.png"))
...
ggplot(Z_max_plot, aes_string(x = "group", y = "negative_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_negative_subtypes.png"))
...
ggplot(Z_max_plot, aes_string(x = "group", y = "absolute_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_absolute_subtypes.png"))
...
For the following analyses and visualisation in python we use a fixed threshold so we need the p_values. Then for the sensitivity analysis we also use a individual threshold with FDR correction so we also calculate the FDR corrected p values.
...
Prepare data
We need to work with the Z_df file since those contain the information about significance. However, we need a wide format instead of a long format so we can loop through the ROIs and count all parcitipants together. We already did this in python so we'll load the data and use that. 
```{r}
...
## Comparisons with TCtest (10%)
Load sub IDs that are in the TCtest set (with 10% of TC used for test)
```{r}
subids_test_TC10 <- read.csv("/Volumes/methlab/Students/Jente/Scripts_ASD_analysis_final/python_scripts/ASD_model/subids_test_TC10.csv")
...
for (i in 1:150){
  ...
  if (p_values[i, 3] < 0.05) {
    ...
    print(
      ggplot(data_roi, aes(x = "classification_model", y = data_megamodel[,p_values$ROI[i]])) +
        geom_boxplot(aes(fill = classification_model)) +
        theme_classic() +
        labs(x = "Classification", y = paste(p_values$ROI[i], "deviation score"), title = paste(p_values$ROI[i], "for each Subgroup")) +
        theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
...
for (i in 1:150){
  ...
  if (p_values_diag[i, 3] < 0.05) {
    ...
    print(
      ggplot(data_roi, aes(x = DX_GROUP, y = data_megamodel_all[,p_values_diag$ROI[i]])) +
        geom_boxplot(aes(fill = DX_GROUP)) +
        theme_classic() +
        labs(x = "Group", y = paste(p_values_diag$ROI[i], "deviation score"), title = paste(p_values_diag$ROI[i], "for TC and Autism")) +
        theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
...
data_megamodel_subs$atyp_index <- 0
...
for (i in 1:nrow(data_megamodel_subs)) {
  ...
  top1 <- subdata[abs(subdata) > absolute_1pct_cutoff]
  data_megamodel_subs$atyp_index[i] <- mean(abs(top1), na.rm = TRUE)
}


# Compare subgroups

wilcox.test(data_megamodel_subs$atyp_index[data_megamodel_subs$classification_model==1], data_megamodel_subs$atyp_index[data_megamodel_subs$classification_model==2])

ggplot(data_megamodel_subs, aes(x = classification_model, y = atyp_index)) +
  geom_boxplot(aes(fill = classification_model)) +
  theme_classic() +
  labs(x = "Group", y = "Atypicality index absolute deviations", title = "Absolute Atypicality index differences between Subgroups") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  geom_jitter(size = 0.1) +
  scale_fill_manual(
    values = c("#5b78e2", "#b30d2e", "#a82393"), name = "Subgroups", labels =
...
for (i in 1:nrow(data_megamodel_all)) {
  ...
  absolute_1pct_cutoff <- quantile(abs(subdata), 0.99, na.rm = TRUE)
  top1 <- subdata[abs(subdata) > absolute_1pct_cutoff]
  data_megamodel_all$atyp_index[i] <- mean(abs(top1), na.rm = TRUE)
}
...
wilcox.test(data_megamodel_all$atyp_index[data_megamodel_all$DX_GROUP==1], data_megamodel_all$atyp_index[data_megamodel_all$DX_GROUP==2])

ggplot(data_megamodel_all, aes(x = DX_GROUP, y = atyp_index)) +
  geom_boxplot(aes(fill = DX_GROUP)) +
  theme_classic() +
  labs(x = "Group", y = "Atypicality index absolute deviations", title = "Absolute Atypicality index differences between diagnosis") +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  geom_jitter(size = 0.1) +
  scale_fill_manual(
    values = c("#5b78e2", "#b30d2e", "#a82393"), name = "Group", labels =
