...
This script is a final version. In the first script we also explored more options than we show in this script. 
This scrip contains a mistake so do not use that for any results. 

This script also works with the data from the new version of FreeSurfer. 
The previous version (Script_clean), works with two different FreeSurfer versions.
...
In this part of the script, the dataframes are prepared and the clustering is performed. 
## Prepare data
...
First select the variables we will need and build a data frame with only
these variables
...
### Change minus numbers to NA

In the data there are some extreme negative values (-9999). Change
these numbers to NAs.
...
### Remove participants with unreliable data
...
# change names for this since we will change the column names. We don't want that to change for the original dataframe.
...
We will remove all women due to uneven distribution of women across sites. 

```{r remove women, cache=TRUE}
# Filters out data where sex=1 which is male according to the legend.
ABIDE_pheno_clean <- filter(ABIDE_pheno_clean, ABIDE_pheno_clean$SEX == 1) # ABIDE com
```

We can't subgroup TC since they don't have any scores for the clinical
...
### Normalize / standardize clinical scores for classification
...
### Get residuals using a GLM

First remove all rows with NA's for any of the needed variables.
```{r create dataframe clustering, cache=TRUE}
dataforclust <- na.omit(dataforclust[, c("SITE_ID", "SUB_ID", "AGE_AT_SCAN", "SEX", "FIQ", "ADOS_MODULE", "ADOS_COMM_zscore", "ADOS_SOCIAL_zscore", "ADOS_STEREO_BEHAV_zscore")])
```

perform the GLM to remove any confounding factors.
```{r GLM, cache=TRUE}
...
Here we perform the model-based clustering based on the clinical subscores.
```{r model cluster, fig.height=5, fig.width=5, cache=TRUE}
...
Then we save the results of the clustering in the dataframe.
```{r save model cluster , cache=TRUE}
dataforclust$classification_model <- as.character(clust_test$classification)
```

#### Visualize these new clusters
...
ggsave("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Cluster plots/Combined_Boxplot_Subscores.png")
```

Lets also make some plots that show the distribution of the variables. Not shown in thesis. 
```{r model cluster plot, fig.height=10, fig.width=10, cache=TRUE}

# Change classification to character
dataforclust$classification_model <- as.character(dataforclust$classification_model)

#Plot 
ggpairs(dataforclust,
  columns = c("ADOS_COMM_zscore_res", "ADOS_SOCIAL_zscore_res", "ADOS_STEREO_BEHAV_zscore_res"),
  ggplot2::aes(colour = classification_model), progress = FALSE,
  columnLabels = c("Communication", "Social interaction", "Stereotyped behaviour"),
  upper = list(continuous = "blank"),
  switch = "both",
  title = "                                 Normalised subscores of the ADOS
                              "
) +
  scale_fill_manual(values = c("#5b78e2", "#b30d2e")) +
  scale_colour_manual(values = c("#5b78e2", "#b30d2e")) +
  theme(strip.background = element_rect(fill = "white"), strip.placement = "outside", text = element_text(size = 18))

ggsave("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Cluster plots/3plots_Subtypes_Subscores.png")
```
...
#### Replicate model based clustering by removing some datapoints and see if we find the same clusters.

To see if the clusters are stable, we remove some random datapoints. If
the clusters change a lot, this could indicate very instable clusters.

Randomly remove 10% of data and run algorithm again. (this is around
24.9 so we will round it up)

We will do this a 1000 times to see how often the same number of
clusters are found.

```{r stability analysis cluster, cache=TRUE}
set.seed(1)

results_stability <- data.frame(matrix(ncol = 3, nrow = 0)) #Make empty dataframe
colnames(results_stability) <- c("Number iteration", "Cluster algorithm", "Optimum number of clusters found") #Change column names

#loop so we can do this process 1000 times.
for (i in 1:1000) {
  results_stability[i, 1] <- i 

  ID_remove <- sample_n(dataforclust, size = (ceiling(length(dataforclust$SUB_ID) * 0.1)))
  ID_remove <- ID_remove$SUB_ID # Determine the sub_ids for the participants that we randomly remove

  dataforclust_removed <- filter(dataforclust, !dataforclust$SUB_ID %in% ID_remove) # Remove the 10% random datapoints

  # use cluster algorithm
  clust_test <- Mclust(dataforclust_removed[, c("ADOS_COMM_zscore_res", "ADOS_SOCIAL_zscore_res", "ADOS_STEREO_BEHAV_zscore_res")], verbose = FALSE)

  # optimal selected model
  results_stability[i, 2] <- clust_test$modelName

  # optimal number of cluster
  results_stability[i, 3] <- clust_test$G
}
```

*How often is each number of clusters found*

```{r results stability, cache=TRUE}
table(results_stability$`Optimum number of clusters found`)  # Show result stability analysis
```
...
ggsave("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Cluster plots/Demo_age_subtypes.png")
...
ggsave("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Cluster plots/Demo_IQ_subtypes.png")
...
ggsave("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Cluster plots/Demo_modules_subtypes.png")
...
Now that we finished our clustering, we can use brain data for the normative model.
The normative model will be done in python. However, we will here start preparing the data. 
Then we load the results from the normative model and perform analysis in following sections.
...
# ABIDE1
sub_keep <- EULER_ABIDE1$SUB_ID[EULER_ABIDE1$meanEULER > -300] # Create vector of subject numbers that are okey to keep.
CT_Destrieux_ABIDE1 <- filter(CT_Destrieux_ABIDE1, CT_Destrieux_ABIDE1$SUB_ID %in% sub_keep) #Filter on subjects you want to keep

# ABIDE 2
sub_keep <- EULER_ABIDE2$SUB_ID[EULER_ABIDE2$meanEULER > -300] # Create vector of subject numbers that are okey to keep.
CT_Destrieux_ABIDE2 <- filter(CT_Destrieux_ABIDE2, CT_Destrieux_ABIDE2$SUB_ID %in% sub_keep) #Filter on subjects you want to keep
...
```{r export data NM}
write.table(data_normative_Destrieux, "/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/Data/Data_normative_Destrieux.txt", append = FALSE, sep = ",", dec = ".", col.names = TRUE)
```
...
Deviation_scores <- read_csv("/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/results/blr/powell/outputs/Deviation_scores.csv", col_types = cols(...1 = col_skip()))
```
...
We will make a dataframe that we will use in the classification in python (with the SVM).
```{r dataframe SVM}
...
write.table(Data_SVM, "/Volumes/methlab/Students/Jente/Scripts/python_scripts/SVM/Data.csv", append = FALSE, sep=",", dec=".", col.names = TRUE)
```
...
We compare the deviation scores from the normative model between the subgroups. We will only show and save the plot if the area is signficiant after FDR correction.
...
#Plot if ROI is significant, otherwise not and show message about significance.
  if (p_values[i, 3] < 0.05) {
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/", p_values$ROI[i],"Sig_devscores_subtypes.png"))
  }
}
...
for (i in 1:150){
  ...
  if (p_values_diag[i, 3] < 0.05) {
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/", p_values_diag$ROI[i],"Sig_devscores_diagnosis.png"))
  }
}
```
...
Z_df <- read.csv("/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/results/Z_df.csv")
...
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "positive_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_positive_diag.png"))
...
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "negative_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_negative_diag.png"))
...
ggplot(Z_max_diagnosis_plot, aes_string(x = "group", y = "absolute_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_absolute_diag.png"))

```

### Compare subtypes

Split into ASD subgroups so we can compare the abnormality index between
these groups.
...
ggplot(Z_max_plot, aes_string(x = "group", y = "positive_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_positive_subtypes.png"))
...
ggplot(Z_max_plot, aes_string(x = "group", y = "negative_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_negative_subtypes.png"))
...
ggplot(Z_max_plot, aes_string(x = "group", y = "absolute_max")) +
  ...
  scale_fill_manual(
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_abnormalindex_absolute_subtypes.png"))
...
For the main analysis we use a fixed threshold so we need the p_values. Then for the sensitivity analysis we also use a individual threshold with FDR correction so we also calculate the fdr corrected p values.
...
#Perform FDR correction for each participant separately. Used in sensitivity analysis.
Z_df <- Z_df %>%
  group_by(SUB_ID) %>%
...
ggplot(counts_all, aes_string(x = "DX_GROUP", y = "counts")) +
  ...
  scale_fill_manual(values = c("#5b78e2", "#b30d2e"), name = "Group")
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_count_diagnosis.png"))


## Compare between ASD subgroups
...
ggplot(counts_ASD, aes_string(x = "classification_model", y = "counts")) +
  ...
  scale_fill_manual(values = c("#5b78e2", "#b30d2e"), name = "Subgroups")
    ...
    ggsave(paste("/Volumes/methlab/Students/Jente/Scripts/Rstudio scripts/Plots/Statistics NM_new/","dif_count_subtypes.png"))
    
    
    
## Compare between subgroups and TC
...
write.table(Z_df, "/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/results/P_df.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)
...
write.table(Z_df_1, "/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/results/Z_df_subtype1.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)
write.table(Z_df_2, "/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/results/Z_df_subtype2.csv", append = FALSE, sep = ",", dec = ".", col.names = TRUE)
```


# Plots for methods
Here we make a plot for the extreme value statistics. We choose one randomly that looks nice. 
...
## Prepare data for mega model # TODO! Changed this code so if we use it add back in
Here we prepare the data that we used for the megamodel. 
```{r}

```


## Do comparisons with TCtest (10%)
Load sub IDs that are in the TCtest set (with 10% of TC used for test)
```{r}
subids_test_TC10 <- read.csv("/Volumes/methlab/Students/Jente/Scripts/python_scripts/ASD_model_new/subids_test_TC10.csv")
subids_TCtest10 = subids_test_TC10$SUB_ID
```
...
for (ROI in colnames(Brain_data_results_all)[16:165]) { 
  ...
  data_roi[, ROI] <- abs(data_roi[, ROI])
  data_roi_1 <- filter(data_roi, data_roi$DX_GROUP == 1)
  data_roi_2 <- filter(data_roi, data_roi$SUB_ID %in% subids_TCtest10)
...
## Abornamlity index 
Z_max_TCtest10 = filter(Z_max, Z_max$SUB_ID %in% subids_TCtest10)
Z_max_TCtest10$group <- "TC"

t_test_positive <- wilcox.test(Z_max_ASD$positive_max, Z_max_TCtest10$positive_max)
t_test_negative <- wilcox.test(Z_max_ASD$negative_max, Z_max_TCtest10$negative_max)
t_test_absolute <- wilcox.test(Z_max_ASD$absolute_max, Z_max_TCtest10$absolute_max)
...
```{r}
wilcox.test(counts_all$counts[counts_all$DX_GROUP == 1], counts_all$counts[counts_all$SUB_ID %in% subids_TCtest10])
```


## Analysis with FDR correction as threshold
```{r}
# Count the number of significant areas
counts <- data.frame(matrix(ncol = 0, nrow = length(unique(Z_df$SUB_ID))))
counts$SUB_ID <- unique(Z_df$SUB_ID)
